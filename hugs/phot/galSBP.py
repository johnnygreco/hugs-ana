...
"""
Wrapper of ELLIPSE.
*** Original code written by Song Huang 
*** Modified by Johnny Greco
"""
...
def writeEllipPar(cfg, image, outBin, outPar, inEllip=None):
    ...
    # ----------------------------------------------------------------- #
    """Sampling parameters"""
    intMode = cfg['integrmode'][0].decode()
...
def readEllipseOut(outTabName, pix=1.0, zp=27.0, exptime=1.0, bkg=0.0,
    ...
    ellipseOut.add_column(Column(name='pa_norm',
                          data=np.array([sUtil.normAngle(pa,
...
def ellipseGetGrowthCurve(ellipOut, bkgCor=False, intensArr=None,
    ...
    if not useTflux:
        ...
        curveOfGrowth = np.asarray(list(map(lambda x: np.nansum(isoFlux[0:x + 1]),
...
def ellipseGetAvgCen(ellipseOut, outRad, minSma=2.0):
    ...
    avgCenX = sUtil.numpy_weighted_mean(xUse, weights=iUse)
    avgCenY = sUtil.numpy_weighted_mean(yUse, weights=iUse)
...
def ellipseGetAvgGeometry(ellipseOut, outRad, minSma=2.0):
    ...
    avgQ = 1.0 - sUtil.numpy_weighted_mean(eUse, weights=fUse)
    avgPA = sUtil.numpy_weighted_mean(pUse, weights=fUse)
...
def ellipseGetOuterBoundary(ellipseOut, ratio=1.2, margin=0.2, polyOrder=12,
    ...
    try:
        ...
        if (negRad is np.nan) or (len(negRad) < 3):
            ...
            try:
                intensFit = sUtil.polyFit(ellipseOut['rsma'][indexUse],
...
def ellipsePlotSummary(ellipOut, image, maxRad=None, mask=None, radMode='rsma',
    ...
    if useZscale:
        try:
            imin, imax = zscale(imgMsk, contrast=0.25, samples=500)
        except Exception:
...
def saveEllipOut(ellipOut, prefix, ellipCfg=None, verbose=True,
    ...
    """ Save a Pickle file """
    if pkl:
        sUtil.saveToPickle(ellipOut, outPkl)
    ...
    if cfg:
        if ellipCfg is not None:
            sUtil.saveToPickle(ellipCfg, outCfg)
...
if __name__ == '__main__':
    ...
    isophote = os.path.join(file_dir, 'bin/x_isophote.e')
    xttools = os.path.join(file_dir, 'bin/x_ttools.e')
